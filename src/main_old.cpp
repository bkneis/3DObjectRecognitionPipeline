...
//  }
  ...
  // Classify the global descriptor
  double result;
  std::vector<std::pair <std::string, std::vector<float> > > vfh_matrix; // people to match against
  // Load global histograms into vfh_matrix
  std::vector<std::string> filenames;

  filenames.push_back("target_globaldesc.pcd");
  filenames.push_back("not_me_globaldesc.pcd");

  for (auto &filename : filenames) {
    GlobalDescriptorsPtr tmp_cloud(new pcl::PointCloud<pcl::VFHSignature308> ());
    pcl::io::loadPCDFile (filename, *tmp_cloud);
    std::vector<float> data;

    for (int i = 0; i < VFH_SIZE; i++)
      data.push_back(tmp_cloud->points[0].histogram[i]);

    vfh_matrix.push_back(std::pair<std::string, std::vector<float> >(filename, data));
  }

  unsigned long models_count = vfh_matrix.size();

  for (int x = 0; x < models_count; ++x) {
    result = ChiSquareDistance(global_descriptor->points[0].histogram, vfh_matrix[x].second.data());
    std::cout << "model: " << vfh_matrix[x].first << " result: " << result << std::endl;
  }
...
//    }
  ...
  else
  {
    ...
    if (global_descriptor)
    {
      hist_vis.addFeatureHistogram (*global_descriptor, VFH_SIZE, "Global descriptor");
    }
    vis.resetCamera ();
    vis.spin ();
  }
